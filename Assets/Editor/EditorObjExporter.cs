...
public class EditorObjExporter : ScriptableObject
{
		...
		for (int material=0; material < m.subMeshCount; material ++) {
			...
			try
			{
				...
				if (mats[material].mainTexture)
					AssetDatabase.GetAssetPath(mats[material].mainTexture);
	...
	private static void MeshToFileNoFilter(Mesh m, string folder, string filename) 
	{
		Dictionary<string, ObjMaterial> materialList = PrepareFileWrite();
		...
		using (StreamWriter sw = new StreamWriter(folder +"/" + filename + ".obj")) 
		{
			...
			sw.Write(MeshToStringNoFilter(m, materialList));
		}
		
		MaterialsToFile(materialList, folder, filename);
	}

	[MenuItem ("Custom/Export/Export Baked Navmesh")]
	static void ExportBakedNavmeshToObj()
	{
		...
		MeshToFileNoFilter(mesh, targetFolder, "navmesh");
		//AssetDatabase.CreateAsset(mesh, "navmesh.fbx");
